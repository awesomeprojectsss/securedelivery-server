...
The repository currently calls the centralized documentation CI from `securedelivery-main`. Do not claim to lint, test or build the NestJS application until it is initialized.
...
Activation material is accepted only in JSON request bodies at the canonical validate, confirm and credential-exchange endpoints. Never place it in paths or query strings. After authenticated Customer confirmation, the Device exchanges it once for a Device bearer credential scoped to that `deviceId`. Human and Device authentication are separate contexts. Redact activation material and issued credentials from logs, traces and errors.
...
Normal period summaries require:
...
Canonical structured summary fields:
...
Telemetry schema version 3 also requires `navigation.status` and `navigation.source`. Metrics may be `null` when unreliable; never convert missing data to zero. Enforce the canonical VALID/PARTIAL/UNAVAILABLE invariants and aggregate only reliable numeric values.
...
Evidence, detector metadata, `attributes` and explicit nullable `location` are required envelope fields. Empty attributes and null location are valid; fabricated context is not.
...
`monitoringSessionId` is generated by the Device before monitoring starts and is canonical on both sides. Session creation is idempotent, and stop synchronization preserves the Device-observed `finishedAt`.
...
Authenticate human socket connections and authorize every emitted resource by role and Customer ownership. A Customer must never receive another tenant's Device, event, ticket or notification signal.
...
Ticket messages must be readable through paginated HTTP history. Resolve and close are explicit lifecycle actions.

Persisted Notifications must be listable and markable as read through HTTP; `notification.created` is only a signal.
